...
const outputDir = path.resolve(__dirname, '..', '页面快照');
...
async function main() {
  ...
  for (const p of pages) {
    ...
    // KEY: Expand the layout to show full content height
    // The layout uses h-screen + overflow-hidden on the root, and overflow-y-auto on main.
    // We need to:
    // 1. Remove height constraint and overflow-hidden from root container
    // 2. Remove overflow-y-auto from main (let it expand naturally)
    // 3. Measure the full content height
    // 4. Resize viewport to fit everything
    ...
    // Take screenshot of the full expanded page
    ...
    // Restore layout styles for next navigation
    await page.evaluate(() => {
      const root = document.querySelector('.flex.h-screen.overflow-hidden') ||
  ...
  await browser.close();
  console.log('\nAll full-page screenshots captured successfully!');
}
